...
; Monitor's stack
SUPER_STACK EQU $FFFFE	        ; top of supervisor stack, monitor worst-case stack usage is about 40-50 bytes
...
; When running from RAM, this is just wasted space
   ...
   DC.L  SUPER_STACK              ; 4 bytes, if executing from RAMBASE. This is also the stack pointer value after reset
   dc.l SRESET
...
initUart:
				 ...
				 move.b  #$93, MRA
        			 move.b  #$07, MRA
				 move.b  #$60, ACR       | Baud Rate Set #2

				 move.b CRA,D0

        			 move.b  #$66, SRA      | Set Tx and Rx rates to 9600
        			 move.b #$0,CUR
        			 move.b #$2,CLR
        			 move.b OPR,d0
...
CINS      btst    #0,  SRA     * Check if receiver ready bit is set
    beq     CINS
    move.b  RBA, d0
          RTS


CIN      btst    #0,  SRA     * Check if receiver ready bit is set
    beq     CIN
    move.b  RBA, d0
         BSR COUT
         RTS
